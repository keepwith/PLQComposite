...
def check_cutoff(plq_loss):
    ...
    for i in range(plq_loss.n_pieces - 1):
        if plq_loss.quad_coef['a'][i] != 0:  # only will happen when the quadratic term is not zero
            cutpoints = -plq_loss.quad_coef['b'][i] / (2 * plq_loss.quad_coef['a'][i])
            if plq_loss.cutpoints[i] < cutpoints < plq_loss.cutpoints[i + 1]:  # if the cutoff is between the knots
                # add the cutoff to the knot list and update the coefficients
                plq_loss.cutpoints.add(cutpoints, i)
